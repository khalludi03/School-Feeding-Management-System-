...
<div class="mt-6 max-w-3xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
    ...
    <form method="post" action="{{ $school ? route('schools.update', $school) : route('schools.store') }}" class="mt-8 space-y-6">
        ...
        @unless($school)
            ...
            <div><label for="participation_starts_on" class="mb-2 block text-sm font-semibold">Participation start date</label><input id="participation_starts_on" name="participation_starts_on" type="date" required value="{{ old('participation_starts_on', today()->toDateString()) }}" class="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100"><p class="mt-1 text-xs text-slate-500">Future dates are allowed. The school will not be selectable for delivery before this date.</p>@error('participation_starts_on')<p class="mt-1 text-sm text-rose-700">{{ $message }}</p>@enderror</div>
        @else
            <p class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">Enrolment and participation dates are changed from the school details page, using the dated workflows that keep earlier demand intact. This form never overwrites that history.</p>
        @endunless

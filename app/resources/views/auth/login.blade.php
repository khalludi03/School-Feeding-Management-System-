...
<div class="mx-auto mt-8 max-w-md rounded-3xl border border-white/80 bg-white/90 p-6 shadow-2xl shadow-blue-900/10 backdrop-blur-xl sm:mt-14 sm:p-9">
    ...
    @endif
    <form class="mt-7 space-y-5" method="post" action="{{ route('login.submit') }}">
        @csrf
        <div>
            <label for="username" class="mb-2 block text-sm font-semibold">Username</label>
            <input id="username" name="username" type="text" autocomplete="username" required autofocus value="{{ old('username') }}" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 outline-none transition focus:border-blue-600 focus:ring-4 focus:ring-blue-100">
        </div>
        <div>
            <label for="password" class="mb-2 block text-sm font-semibold">Password</label>
            <input id="password" name="password" type="password" autocomplete="current-password" required class="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 outline-none transition focus:border-blue-600 focus:ring-4 focus:ring-blue-100">
        </div>
